...
export class ProfessionComponent implements OnInit, OnDestroy {
  ...
  private setupChart(): void {
    ...
    this.zone.runOutsideAngular(() => {
      const root: am5.Root = am5.Root.new("professionChartDiv");
      ...
      const transformData = (data: any[], map: { [key: string]: string }): { id: string, name: string, value: number }[] => {
        return data.map(item => {
          ...
          return {
            id: countryCode || "",
            name: item.nom,
            value: this.getprofessionsByJuridictionPosteId(item.poste.id)
          };
        });
      };
  ...
  private getprofessionsByJuridictionPosteId(posteId: number): number {
    return this.professions.filter(profession => profession.consulat?.poste?.id === posteId).length;
  }
